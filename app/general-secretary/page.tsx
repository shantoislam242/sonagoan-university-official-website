import { buildMetadata } from '@/lib/metadata';
import HeaderUniversity from '@/components/layout/HeaderUniversity';
import FooterUniversity from '@/components/layout/FooterUniversity';
import PageBanner from '@/components/sections/PageBanner';
import Container from '@/components/ui/Container';
import Link from 'next/link';
import { Quote, ArrowUpRight } from 'lucide-react';

export const metadata = buildMetadata({
  title: 'General Secretary, SU Trust',
  description:
    'Message from Engr. Md. Abdul Alim, Co-Founder and General Secretary of the Sonargaon University Trust.',
  path: '/general-secretary',
});

const PARAGRAPHS = [
  'When we founded Sonargaon University in 2012, we shared a simple conviction: that quality higher education should not be a privilege of the few, but within the reach of every deserving student.',
  'As a registered, non-political, non-profit Trust, we set out to build an institution where talented young people, regardless of their economic means, could learn, grow, and thrive. Today, seeing thousands of graduates carry the name of Sonargaon University into their professions and communities is the greatest reward of that vision.',
  'On behalf of the Sonargaon University Trust, I welcome you to be part of our journey, and to help us build a world of good, together.',
];

export default function GeneralSecretaryPage() {
  return (
    <>
      <HeaderUniversity />
      <PageBanner
        title="General Secretary, SU Trust"
        breadcrumb={[
          { label: 'Home', href: '/' },
          { label: 'About', href: '/about' },
          { label: 'General Secretary' },
        ]}
      />

      {/* Hero — portrait + identity */}
      <section style={{ background: '#ffffff', padding: '80px 0 60px' }}>
        <Container className="!max-w-[1200px]">
          <div className="gs-hero">
            <div className="gs-hero__media">
              <span className="gs-hero__tag">Founder</span>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/assets/images/leadership/abdul-alim.jpeg" alt="Engr. Md. Abdul Alim" className="gs-hero__img" />
            </div>
            <div className="gs-hero__text">
              <span style={{ display: 'inline-block', color: '#CC1579', fontWeight: 700, fontSize: 13, letterSpacing: '0.18em', textTransform: 'uppercase', marginBottom: 14 }}>
                Leadership
              </span>
              <h2 style={{ fontFamily: 'var(--font-poppins), Poppins, sans-serif', fontWeight: 700, color: '#2B3175', fontSize: 'clamp(30px, 4vw, 48px)', lineHeight: 1.15, marginBottom: 14 }}>
                Engr. Md. Abdul Alim
              </h2>
              <p style={{ color: '#2B3175', fontSize: 18, fontWeight: 600, margin: '0 0 6px' }}>
                General Secretary, Sonargaon University Trust
              </p>
              <p style={{ color: '#5B5B78', fontSize: 16, margin: '0 0 22px' }}>
                Co-Founder, Sonargaon University
              </p>
              <p style={{ color: '#5B5B78', fontSize: 16.5, lineHeight: 1.85, margin: 0 }}>
                Engr. Md. Abdul Alim is a co-founder of Sonargaon University and serves as the General
                Secretary of the Sonargaon University Trust &mdash; a registered, non-political, non-profit
                voluntary organization established in 2012 to make affordable, quality higher education
                accessible to students across Bangladesh.
              </p>
            </div>
          </div>
        </Container>
      </section>

      {/* Message */}
      <section style={{ background: '#F6F7FC', padding: '80px 0 90px' }}>
        <Container className="!max-w-[1240px]">
          <div className="gs-letter">
            {/* left brand panel */}
            <aside className="gs-letter__aside">
              <span className="gs-letter__badge"><Quote size={26} /></span>
              <p className="gs-letter__quote">
                Quality higher education should not be a privilege of the few, but within the reach of every
                deserving student.
              </p>
              <div className="gs-letter__sign">
                <p className="gs-letter__sign-name">Engr. Md. Abdul Alim</p>
                <p className="gs-letter__sign-role">Co-Founder &amp; General Secretary, SU Trust</p>
              </div>
            </aside>

            {/* message body */}
            <div className="gs-letter__body">
              <span className="gs-letter__eyebrow">Message</span>
              <h3 className="gs-letter__title">General Secretary&rsquo;s Message</h3>
              {PARAGRAPHS.map((p, i) => (
                <p key={i} className="gs-letter__p">{p}</p>
              ))}
              <p className="gs-letter__closing">Let us build a world of good, together.</p>
              <Link href="/board-of-trustees" className="gs-cta">
                Meet the Board of Trustees <ArrowUpRight size={18} />
              </Link>
            </div>
          </div>
        </Container>
      </section>

      <FooterUniversity />

      <style>{`
        .gs-hero { display: grid; grid-template-columns: 1fr; gap: 40px; align-items: center; }
        @media (min-width: 860px) { .gs-hero { grid-template-columns: 380px 1fr; gap: 56px; } }
        .gs-hero__media {
          position: relative; border-radius: 20px; overflow: hidden; max-width: 380px; margin-left: auto; margin-right: auto;
          box-shadow: 0 24px 60px rgba(43,49,117,0.18); border: 1px solid #ECECF3;
        }
        .gs-hero__img { display: block; width: 100%; height: auto; object-fit: cover; }
        .gs-hero__tag {
          position: absolute; top: 16px; left: 16px; z-index: 2;
          background: linear-gradient(135deg, #2B3175, #CC1579); color: #fff;
          font-size: 12px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase;
          padding: 7px 16px; border-radius: 999px; box-shadow: 0 6px 16px rgba(0,0,0,0.18);
        }

        .gs-letter {
          display: grid; grid-template-columns: 1fr; overflow: hidden;
          background: #ffffff; border: 1px solid #ECECF3; border-radius: 22px;
          box-shadow: 0 24px 60px rgba(43,49,117,0.08);
        }
        @media (min-width: 860px) { .gs-letter { grid-template-columns: 380px 1fr; } }

        .gs-letter__aside {
          background: linear-gradient(160deg, #2B3175 0%, #3a2c74 55%, #CC1579 140%);
          color: #fff; padding: clamp(32px, 4vw, 48px);
          display: flex; flex-direction: column;
        }
        .gs-letter__badge {
          display: inline-flex; align-items: center; justify-content: center;
          width: 58px; height: 58px; border-radius: 16px; margin-bottom: 26px;
          background: rgba(255,255,255,0.14); color: #fff;
        }
        .gs-letter__quote {
          font-family: var(--font-poppins), Poppins, sans-serif; font-weight: 600; font-style: italic;
          font-size: clamp(19px, 2.2vw, 24px); line-height: 1.55; margin: 0 0 32px; color: #fff;
        }
        .gs-letter__sign { margin-top: auto; padding-top: 22px; border-top: 1px solid rgba(255,255,255,0.2); }
        .gs-letter__sign-name {
          font-family: var(--font-poppins), Poppins, sans-serif; font-weight: 700; color: #fff;
          font-size: 19px; margin: 0 0 4px;
        }
        .gs-letter__sign-role { color: rgba(255,255,255,0.78); font-size: 13.5px; margin: 0; }

        .gs-letter__body { padding: clamp(32px, 4.5vw, 56px); }
        .gs-letter__eyebrow {
          display: inline-block; color: #CC1579; font-weight: 700; font-size: 12.5px;
          letter-spacing: 0.16em; text-transform: uppercase; margin-bottom: 10px;
        }
        .gs-letter__title {
          font-family: var(--font-poppins), Poppins, sans-serif; font-weight: 700; color: #2B3175;
          font-size: clamp(24px, 3vw, 32px); margin: 0 0 22px;
        }
        .gs-letter__p { color: #5B5B78; font-size: 16.5px; line-height: 1.9; margin: 0 0 18px; }
        .gs-letter__closing {
          color: #CC1579; font-family: var(--font-poppins), Poppins, sans-serif; font-weight: 600;
          font-size: 18px; margin: 26px 0 30px;
        }

        .gs-cta {
          display: inline-flex; align-items: center; gap: 9px;
          background: #2B3175; color: #fff !important; font-weight: 600; font-size: 15.5px; text-decoration: none;
          padding: 14px 28px; border-radius: 10px; transition: background .2s ease, gap .2s ease;
        }
        .gs-cta:hover { background: #CC1579; color: #fff !important; gap: 13px; }
        .gs-cta svg { color: #fff; }
      `}</style>
    </>
  );
}
